...
class OtherCertID {
   ...
   */
		...
		return new asn1js.Sequence({
			...
			value: [new asn1js.Choice({
				value: [new asn1js.OctetString({ name: names.otherCertHashSimple || "otherCertHash" }), _OtherHashAlgAndValue2.default.schema(names.otherCertHashObject || {
					names: {
  ...
  */
	fromSchema(schema) {
		...
		if (asn1.result.otherCertHash instanceof asn1js.OctetString) this.otherCertHash = asn1.result.otherCertHash;else this.otherCertHash = new _OtherHashAlgAndValue2.default({ schema: asn1.result.otherCertHash });
  ...
  */
	fillValues(parameters) {
		...
		if ("certificate" in parameters) certificate = parameters.certificate; // in_window.org.pkijs.simpl.CERT
		...
		sequence = sequence.then(() => crypto.digest({ name: hashAlgorithm }, certificate.toSchema().toBER(false)), error => Promise.reject(error)).then(result => {
			if (_this.otherCertHash instanceof asn1js.OctetString) _this.otherCertHash.valueBlock.valueHex = result;else _this.otherCertHash.hashValue = new asn1js.OctetString({ valueHex: result });
