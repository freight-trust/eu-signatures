...
class SignatureTimeStamp extends _pkijs.ContentInfo {
  ...
  */
    ...
    */
			this.tspResponse = (0, _pvutils.getParametersValue)(parameters, "tspResponse", SignatureTimeStamp.defaultValues("tspResponse"));
    ...
    * @description tstInfo
    */
			this.tstInfo = (0, _pvutils.getParametersValue)(parameters, "tstInfo", SignatureTimeStamp.defaultValues("tstInfo"));
		//endregion
  ...
  */
	fromSchema(schema) {
		...
		if (cmsSigned.encapContentInfo.eContentType !== "1.2.840.113549.1.9.16.1.4") throw new Error("Incorrect format for SignatureTimeStamp");

		if ("eContent" in cmsSigned.encapContentInfo === false) throw new Error("Incorrect format for SignatureTimeStamp");

		if (cmsSigned.encapContentInfo.eContent instanceof asn1js.OctetString === false) throw new Error("Incorrect format for SignatureTimeStamp");
  ...
  */
	getStampingBuffer(cmsSignedData, signerIndex, parameters) {
		...
		return Promise.resolve().then(() => crypto.digest({ name: hashAlgorithm }, cmsSignedData.signerInfos[signerIndex].signature.valueBlock.valueHex), error => Promise.reject(error));
	}
	//**********************************************************************************
	/**
  ...
  */
	makeAttribute(parameters = {}) {
		...
		if ("tspResponse" in parameters) tspResponse = parameters.tspResponse;else {
			if ("tspResponse" in this) tspResponse = this.tspResponse;else throw new Error("Parameter \"tspResponse\" is mandatory for making \"signature-time-stamp\" attribute");
		}
