...
function createCommonAttributes(cmsSigned, parameters) {
	...
	if ("certificate" in parameters === false) return Promise.reject("Parameter \"certificate\" is mandatory for making common CAdES attributes");

	if ("contentOID" in parameters) contentOID = parameters.contentOID;else return Promise.reject("Parameter \"contentOID\" is mandatory for making common CAdES attributes");
	//endregion 
	...
	sequence = sequence.then(() => {
		...
		const signingTimeAttribute = new _pkijs.Attribute({
			...
			values: [new asn1js.UTCTime({ valueDate: (0, _pvutils.getUTCDate)(new Date()) })]
		});
	...
	sequence = sequence.then(() => {
		...
		if ("eContent" in cmsSigned.encapContentInfo) // Attached data
			{
				if (cmsSigned.encapContentInfo.eContent.idBlock.tagClass === 1 && cmsSigned.encapContentInfo.eContent.idBlock.tagNumber === 4) {
					if (cmsSigned.encapContentInfo.eContent.idBlock.isConstructed === false) digistedContent = cmsSigned.encapContentInfo.eContent.valueBlock.valueHex;else {
						for (let i = 0; i < cmsSigned.encapContentInfo.eContent.valueBlock.value.length; i++) digistedContent = (0, _pvutils.utilConcatBuf)(digistedContent, cmsSigned.encapContentInfo.eContent.valueBlock.value[i].valueBlock.valueHex);
					}
				} else digistedContent = cmsSigned.encapContentInfo.eContent.valueBlock.valueHex;
			} else // Detached data
			{
				if ("content" in parameters === false) return Promise.reject("Please provide \"content\" parameter");
	...
	sequence = sequence.then(() => essCertificate.fillValues(parameters), error => Promise.reject(error)).then(() => {
