...
class OcspResponsesID {
  ...
  */
	fromSchema(schema) {
		...
		this.ocspIdentifier = new _OcspIdentifier2.default({ schema: asn1.result.ocspIdentifier });

		if ("ocspRepHash" in asn1.result) {
			if (asn1.result.ocspRepHash instanceof asn1js.OctetString) this.ocspRepHash = asn1.result.ocspRepHash;else this.ocspRepHash = new _OtherHashAlgAndValue2.default({ schema: asn1.result.ocspRepHash });
		}
		//endregion
	}
	//**********************************************************************************
  ...
  */
	fillValues(parameters) {
		...
		if ("ocspResponse" in parameters) ocspResponse = parameters.ocspResponse; // in_window.org.pkijs.simpl.OCSP_RESPONSE
		...
		sequence = sequence.then(() => crypto.digest({ name: hashAlgorithm }, ocspResponse.toSchema().toBER(false)), error => Promise.reject(error)).then(result => {
			if (this.ocspRepHash instanceof _OtherHashAlgAndValue2.default) this.ocspRepHash.hashValue = new asn1js.OctetString({ valueHex: result });else this.ocspRepHash = new asn1js.OctetString({ valueHex: result });
