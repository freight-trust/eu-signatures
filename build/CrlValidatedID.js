...
class CrlValidatedID {
  ...
  */
	fromSchema(schema) {
		...
		if ("crlIdentifier" in asn1.result) this.crlIdentifier = new _CrlIdentifier2.default({ schema: asn1.result.crlIdentifier });
		//endregion
	}
	//**********************************************************************************
	/**
  ...
  */
	fillValues(parameters) {
		...
		sequence = sequence.then(() => crypto.digest({ name: hashAlgorithm }, crl.toSchema().toBER(false)), error => Promise.reject(error)).then(result => {
			if (_this._crlHashType === 0) _this.crlHash = new asn1js.OctetString({ valueHex: result });else _this.crlHash.hashValue = new asn1js.OctetString({ valueHex: result });
